...
function maleTweet(id) {
    ...
    httpClient.open("GET", "api/tweets/?id=" + id + "&gender=0", true);
    httpClient.send();
} 

function femaleTweet(id) {
    let httpClient = new XMLHttpRequest();
    httpClient.open("GET", "api/tweets/?id=" + id + "&gender=1", true);
    httpClient.send();
}

function nonbinTweet(id) {
    let httpClient = new XMLHttpRequest();
    httpClient.open("GET", "api/tweets/?id=" + id + "&gender=2", true);
    httpClient.send();
}

function dismiss(id) {
    let httpClient = new XMLHttpRequest();
    httpClient.open("DELETE", "api/tweets?id=" + id, true);
    httpClient.send();
}
...
function buildTweetHtml(tweet) {
    ...
    let basicInfo = text + linkSection + buttonMale + buttonFemale+ buttonNonBinary + dismissButton;
    return basicInfo;
}
